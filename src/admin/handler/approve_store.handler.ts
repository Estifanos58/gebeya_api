...
export class ApproveStoreHandler implements ICommandHandler<ApproveStoreCommand> {
    ...
    async execute(command: ApproveStoreCommand): Promise<any> {
        ...
        try {
            const store = await this.storeRepository.findOne({ where: { id: storeId }, relations: ['user']});
            ...
            if(!isApproved) {
                ...
                this.eventEmitter.emit('store.disapproved', new StoreApproveEvent(store));
                return {
            ...
            if( store.isVerified === isApproved) {
                throw new HttpException({ message: "Store" }, HttpStatus.NOT_MODIFIED)
            }

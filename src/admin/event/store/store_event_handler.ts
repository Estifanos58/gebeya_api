...
import { STORE_APPROVED_TEMPLATE, STORE_BANNED_TEMPLATE, STORE_DISAPPROVED_TEMPLATE } from "@/utils/templates";
    ...
    handleStoreApprovedEvent(payload: StoreApproveEvent) {
        ...
        try {
            ...
            this.activityLogService.error(
                ...
                store.user.email,
                store.user.role,
                { StoreId: store.id, Owner_Email: store.user.email, error: error.message }
            );
        }
    }
        ...
        try {
            ...
            const mail = {
                ...
                placeholders: {
                    name: store.user.firstName,
                    storeName: store.name,
                    reason: 'Your store has been disapproved by the admin. Please contact support for more',

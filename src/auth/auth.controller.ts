...
export class AuthController {
  ...
  async login(@Body() loginDto: loginDto, @Res() res: Response) {
    ...
    const user = await this.commandBus.execute(
      new LoginUserCommand(loginDto.email, loginDto.password, res),
    );
  ...
  async verifyOtp(
    ...
    const otpValid = await this.commandBus.execute(
      new VerifyOtpCommand(user, otp),
    );
  ...
  async forgotPassword(@Body() body: ForgotPasswordDto, @Res() res: Response) {
    const { email } = body;
    const emailSent = await this.commandBus.execute(
  ...
  async resetPassword(
    ...
    const user = await this.commandBus.execute(
      new ResetPasswordCommand(token, email, body.newPassword, res),
    );
  ...
  async refreshToken(@Req() req: Request, @Res() res: Response) {
    const user = await this.commandBus.execute(
      new RefreshTokenCommand(req, res),
    );
